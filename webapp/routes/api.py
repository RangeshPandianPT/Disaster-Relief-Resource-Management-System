...
# ============================================================
# DISASTERS GEO API (FEATURE 1)
# ============================================================

@api.route('/disasters_geo')
def get_disasters_geo():
    """Get geospatial data for disasters for the interactive map."""
    # Since the original schema doesn't have lat/lon, we mock them
    # by generating pseudo-random offsets from a central point (e.g., India center)
    # using the disaster_id to keep them stable.
    status = request.args.get('status')
    ...
    if result:
        for r in result:
            # Base Lat / Lng (approximate center of India)
            base_lat = 20.5937
            base_lng = 78.9629
            
            # Use disaster_id to generate a stable pseudo-random offset
            import math
            offset_lat = math.sin(r['disaster_id'] * 10) * 8.0 # up to +/- 8 degrees
            offset_lng = math.cos(r['disaster_id'] * 10) * 10.0 # up to +/- 10 degrees
